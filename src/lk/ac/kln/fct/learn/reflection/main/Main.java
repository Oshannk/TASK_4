...
public class Main {
	...
	public static Shape createShape(Class<? extends Shape> clazz, Dimension dimension) {
		...
		try {
			...
			Constructor<?> ctor = clazz.getDeclaredConstructor(Dimension.class);
			ctor.setAccessible(true);
			Shape shape = (Shape) ctor.newInstance(dimension);
	...
	public static String setColorAndGetColorName(Shape shape, String name, Color value) {
		...
		try {
			...
			Method setMethod = clazz.getDeclaredMethod(setterName,clazz.getDeclaredField(fieldName).getType());
			setMethod.invoke(shape, value);

			Method getMethod = clazz.getDeclaredMethod(getterName);
			String color = (String) getMethod.invoke(shape);
